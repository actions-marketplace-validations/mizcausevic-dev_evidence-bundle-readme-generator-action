...
function header(m, opts) {
    ...
    if (!opts.hideBadges) {
        const badges = [];
        if (m.bundle.purpose)
            badges.push(PURPOSE_BADGE[m.bundle.purpose] ?? m.bundle.purpose);
    ...
    if (m.bundle.created_at)
        lines.push(`**Created at:** ${m.bundle.created_at}`);
    if (m.evidence_bundle_version)
        lines.push(`**Spec version:** \`${m.evidence_bundle_version}\``);
    return lines.join("\n");
}
function itemsBlock(m, anchorPrefix) {
    const lines = [`## Items (${m.items.length})`, ""];
    if (m.items.length === 0) {
...
function relationshipsBlock(m) {
    const rels = m.relationships ?? [];
    const lines = [`## Relationships (${rels.length})`, ""];
    if (rels.length === 0) {
...
function provenanceBlock(m) {
    ...
    if (p.agent_card_uri)
        lines.push(`- **Agent Card:** ${p.agent_card_uri}`);
    if (p.tool_card_uri)
        lines.push(`- **Tool Card:** ${p.tool_card_uri}`);
    if (p.prompt_provenance_uri)
        lines.push(`- **Prompt Provenance:** ${p.prompt_provenance_uri}`);
    if (p.otel_trace_id)
        lines.push(`- **OTel trace id:** \`${p.otel_trace_id}\``);
    if (p.model)
        lines.push(`- **Model:** \`${p.model}\``);
    if (p.retrieval_query)
        lines.push(`- **Retrieval query:** ${p.retrieval_query}`);
    return lines.join("\n");
}
function signatureBlock(m) {
    const s = m.signature;
    ...
    if (s.signed_at)
        lines.push(`- **Signed at:** ${s.signed_at}`);
    lines.push(`- **Value:** \`${s.value.slice(0, 24)}…\``);
    return lines.join("\n");
}
function labelsBlock(m) {
...
export function generateReadme(m, opts = {}) {
